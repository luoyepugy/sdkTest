...
define(['./module', '../modules/validate-tips'], function(controllers, messages) {
	controllers.controller('tradeSucceCtrl', ['$scope', '$ionicLoading', 'tradeService', function($scope, $ionicLoading, tradeService) {
	    ...
	    $scope.doRefresh = function() {
	    	var promise = tradeService.getData({'page': 'succe', 'status': 'refresh'}, '../../json/trade-more.json');
		    promise.then(function(data) {
		    	$ionicLoading.hide();
		    	$scope.list = data;
		    	lastId = data[data.length-1].id;
	    ...
	    $scope.loadMore = function() {
	    	var promise = tradeService.getData({'page': 'succe', 'status': 'loadmore', 'id': lastId}, '../../json/trade-more.json');
		    promise.then(function(data) {
