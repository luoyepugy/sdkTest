		        ...
		        scope.getData = function(name) {
		            ...
		            if (top===index*36) {
		            	scope.dataing = $timeout(function() {
		            		if(province){
		            			scope.province = scope.cityData[index];
		            			scope.city = scope.province.sub[0];
		            			scope.country = {};
		            			if(scope.city && scope.city.sub) {
		            				scope.country = scope.city.sub[0];
		            			}
		            		}
		            		if(city) {
		            			scope.city = scope.province.sub[index];
		            			scope.country = {};
		            			if(scope.city && scope.city.sub) {
		            				scope.country = scope.city.sub[0];
		            			}
		            		...
		            		if(country) {
		            			scope.country = scope.city.sub[index];
		            		}
		            		//数据同步
